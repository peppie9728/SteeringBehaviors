namespace PaperPlaneTools.AR {
	...
	public class MarkerDetector {
		...
		public List<int> Detect(Mat mat, int width, int height) {
            ...
            // Gidi: I removed the comment below, to see the corners+id around markers
			CvAruco.DrawDetectedMarkers (mat, corners, ids);

			float markerSizeInMeters = 1f;

			Point3f[] markerPoints = new Point3f[] {
				new Point3f(-markerSizeInMeters / 2f,  markerSizeInMeters / 2f, 0f),    // Gidi: top left corner in object coordinates?
				new Point3f( markerSizeInMeters / 2f,  markerSizeInMeters / 2f, 0f),    // Gidi: top right corner
				new Point3f( markerSizeInMeters / 2f, -markerSizeInMeters / 2f, 0f),    // Gidi: bottom right corner
				new Point3f(-markerSizeInMeters / 2f, -markerSizeInMeters / 2f, 0f)     // Gidi: bottom left corner
			};
...
//			
			...
			for (int i=0; i<ids.Length; i++) {
                ...
                // Gidi: next line draws axis and another horizontal line?
                // Gidi: rvec is a rotation vector, tvec is a translation vector
